...
function NavBar() {
    ...
    return (
        <div className="flex justify-between items-center w-full h-20 text-white bg-black fixed px-4">
            <div>
                <h1 className="text-4xl ml-2 font-signature">Magical Khanna</h1>
            </div>
            <ul className="md:flex hidden">

                {links.map(({ id, link }) => (
                    <li key={id} className="capitalize cursor-pointer px-4 font-medium text-gray-500 hover:scale-105 duration-200"><Link to={link} smooth duration={500}>{link}</Link></li>
                ))}
            </ul>
